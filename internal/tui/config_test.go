...
import (
	...
	"metacubexd-server-go/internal/tui/pages/kernel"
...
func TestConfigViewerOpen(t *testing.T) {
	...
	m := New(client.NewClient(srv.URL, "", false))
	nm, _ := m.Update(tea.WindowSizeMsg{Width: 80, Height: 24})
	...
	if !nm.(Model).tabs[4].(*kernel.Model).ViewingConfig() {
...
func TestConfigViewerToggle(t *testing.T) {
	...
	if got := nm.(Model).tabs[4].(*kernel.Model).ConfigMode(); got != "runtime" {
...
func TestSectionEdit(t *testing.T) {
	...
	if !nm.(Model).tabs[4].(*kernel.Model).EditingSection() {
...
func TestNetworkFieldEdit(t *testing.T) {
	...
	nm, _ = nm.Update(tea.KeyPressMsg{Code: tea.KeyEnter})
	if !nm.(Model).tabs[4].(*kernel.Model).Editing() {
		t.Fatal("editing should be true after enter on mixed-port")
	}
	if got := nm.(Model).tabs[4].(*kernel.Model).EditFieldValue(); got != "7890" {
...
func TestTunFieldEdit(t *testing.T) {
	...
	nm, _ = nm.Update(tea.KeyPressMsg{Code: tea.KeyEnter})
	if got := nm.(Model).tabs[4].(*kernel.Model).EditFieldValue(); got != "true" {
