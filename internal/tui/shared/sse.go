package shared

import (
	"context"
	"encoding/json"
	"fmt"
	"time"

	tea "charm.land/bubbletea/v2"

	"metacubexd-server-go/internal/tui/client"
	"metacubexd-server-go/internal/api"
)

// Subscribe opens the SSE log subscription and hands the stream to the model
// via SubscribedMsg.
func Subscribe(c *client.Client) tea.Cmd {
	return func() tea.Msg {
		ctx, cancel := context.WithCancel(context.Background())
		ch, err := c.SubscribeLogs(ctx)
		if err != nil {
			cancel()
			return StatusErrorMsg{Err: err}
		}
		return SubscribedMsg{Ch: ch, Cancel: cancel}
	}
}

// ForwardEvents pumps one event from the stream into the message loop,
// re-arming itself so the stream keeps flowing.
func ForwardEvents(ch <-chan client.Event) tea.Cmd {
	return func() tea.Msg {
		ev, ok := <-ch
		if !ok {
			return LogClosedMsg{}
		}
		if msg := ParseLogEvent(ev); msg != nil {
			return msg
		}
		return ForwardEvents(ch)()
	}
}

// ParseLogEvent decodes an SSE payload into a LogLineMsg or KernelStateMsg,
// or nil for unknown event types.
func ParseLogEvent(ev client.Event) tea.Msg {
	var header struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal([]byte(ev.Data), &header); err != nil {
		return nil
	}
	switch header.Type {
	case "log":
		var l api.KernelLogLine
		if err := json.Unmarshal([]byte(ev.Data), &l); err != nil {
			return nil
		}
		return LogLineMsg{Line: FormatLogLine(l)}
	case "state":
		var st api.KernelState
		if err := json.Unmarshal([]byte(ev.Data), &st); err != nil {
			return nil
		}
		return KernelStateMsg{State: st}
	}
	return nil
}

// FormatLogLine renders a kernel log line as "2006-01-02 15:04:05 LEVEL  line".
func FormatLogLine(l api.KernelLogLine) string {
	level := "INFO "
	if l.Stream == "stderr" {
		level = ErrorStyle.Render("ERROR")
	}
	ts := time.UnixMilli(l.TS).Format("2006-01-02 15:04:05")
	return fmt.Sprintf("%s %s  %s", ts, level, l.Line)
}

// CloseStream cancels the SSE subscription so its goroutine and HTTP
// connection are released on exit.
func CloseStream(cancel context.CancelFunc) {
	if cancel != nil {
		cancel()
	}
}
