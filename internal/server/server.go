// Package server assembles and runs the metacubexd-server. It owns the full
// lifecycle — from directory creation and component construction through HTTP
// serving and graceful shutdown — so that cmd/ only handles CLI parsing.
package server
...
import (
	...
	"metacubexd-server-go/internal/server/auth"
...
// Options holds the configuration needed to start the server.
type Options struct {
...
// Run starts the server and blocks until a shutdown signal is received. It
...
func Run(opts Options) error {
	...
	if err := os.MkdirAll(env.ProfilesDir(), 0o755); err != nil {
		return fmt.Errorf("cannot create profiles dir %s: %w", env.ProfilesDir(), err)
	}
	...
	// The effective mihomo secret: env.ClashSecret verbatim if set, otherwise
	// a random one generated by the supervisor (stored in opts.Secret at New
	// time, before the kernel runs). We read it back here so the auth layer
	// and the proxy agree on the SAME value — a cross-origin caller supplying
	// this secret passes auth AND reaches mihomo. Passing env.ClashSecret
	// directly instead would leave auth with an empty secret when CLASH_SECRET
	// is unset (rejecting all cross-origin) while the proxy injected the
	// random one to mihomo — an invisible source-of-truth split.
	effectiveSecret := sup.State().Secret
	...
	if err != nil {
		return fmt.Errorf("cannot init static handler: %w", err)
	}
	mux.Handle("/", staticHandler)

	// Auth middleware wraps the entire mux. It handles login (/login,
	// /api/auth/*), lets the capability probes through, and validates both
	// credential domains — CONTROL_TOKEN (cookie / Bearer) for /api/control/*
	// and CLASH_SECRET (Bearer / ?token=) or a login cookie for /api/clash/*.
	//
	// Order matters: auth must be OUTSIDE mux so /login isn't swallowed by
	// the static SPA fallback.
	handler := auth.New(auth.Config{
		ControlToken: env.ControlToken,
		ClashSecret:  effectiveSecret,
	...
	go func() {
		...
		if env.ControlToken != "" {
			...
			// With auth on, /api/clash/* accepts either a login cookie or the
			// clash secret. Always print the effective secret (and where it came
			// from) so an operator can wire up a cross-origin panel without
			// digging through logs or guessing whether one was configured.
			if env.ClashSecret != "" {
				log.Printf("[server]   clash secret: %s (from CLASH_SECRET)", effectiveSecret)
			} else {
				log.Printf("[server]   clash secret: %s (auto-generated)", effectiveSecret)
			}
		} else {
			log.Printf("[server]   login auth:  DISABLED (set CONTROL_TOKEN to enable login page)")
			if env.ClashSecret != "" {
				// CLASH_SECRET only matters behind the auth gate (open mode skips
				// all checks, so /api/clash/* is reachable without any secret).
				// This combo almost always means a misconfiguration.
				log.Printf("[server]   WARNING: CLASH_SECRET is set but CONTROL_TOKEN is not — open mode ignores CLASH_SECRET; /api/clash/* is fully open. Set CONTROL_TOKEN to enforce it.")
			}
		}
